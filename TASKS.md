...
- [ ] **Mobile UI redesign** (branch: `ai/claude-mobile-redesign`)
  ...
  - Screen order: 1) base styles + bottom nav/FAB, 2) Home (hero banner, underline tabs, game list rows with circular course thumbs), 3) Create game (chip selectors, tee-time bottom sheet), 4) Game detail (banner header, numbered player list), 5) Profile (centered avatar, stats row, form).
